    ...
    const stats = { calls: 0, latency: 0, trains: 0, buses: 0 };
    let refreshTimer = 30;
    ...
    const i18n = {
        en: {
            ...
            ontime: "ON TIME", favs: "FAVORITES", analytics: "ANALYTICS", config: "CONFIG",
        ...
        no: {
            ...
            ontime: "I RUTE", favs: "FAVORITTER", analytics: "ANALYSE", config: "OPPSETT",
    ...
    function render(calls) {
        ...
        calls.forEach(call => {
            ...
            let status = mode === 'rail' ? `SP ${call.quay?.publicCode || '–'}` : (delay > 0 ? `+${delay}m` : i18n[CONFIG.lang].ontime);
            let statusClass = delay > 0 ? 'color:#ff4444' : 'color:#00ff88';
            row.innerHTML = `
                ...
                <div class="board-dest">${call.destinationDisplay.frontText}</div>
                <div class="status-cell" style="${mode !== 'rail' ? statusClass : ''}">${status}</div>
            `;
    ...
    function toggleModal(id) {
        ...
        if (id === 'stats-modal') {
            document.getElementById('stats-body').innerHTML = `
                ...
                <div class="setting-row"><span>Requests</span><span>${stats.calls}</span></div>
            `;
        }
    }
